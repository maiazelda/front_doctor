...
export const getters = {
  isAuth: state => !!state.user && !state.user.expired && state.user.profile['cognito:groups'].includes('doctor')
}
...
export const actions = {
  ...
  loginCallback({
    ...
    return Auth.instance.signinRedirectCallback().then(user => {
      if (user.profile['cognito:groups'].includes('doctor')) {
  ...
  initFromStorage({ commit }) {
    ...
    return Auth.getUser()
      .then(user => {
        ...
        if (user && !user.profile['cognito:groups'].includes('doctor')) {
          return Auth.instance.removeUser()
            .then(() => {
              return false
            })
          ...
          return !!user && !user.expired && user.profile['cognito:groups'].includes('doctor')
        }
      })
      .catch(error => {
